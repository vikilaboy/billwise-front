...
import {Archive, ArchiveRestore, FileSignature, GitBranch, Pencil, Play, Plus, Search, Square, Trash2} from "lucide-react";
...
type ContractForm = {
  ...
  currency_id: string; payment_terms_days: string; default_hours_per_day: string;
...
type ContractOperation = "activate" | "end" | "archive" | "restore" | "delete";
...
const EMPTY: ContractForm = {
  ...
  currency_id: "", payment_terms_days: "15", default_hours_per_day: "8.00",
...
export function ContractsPage() {
  ...
  const action = useMutation({
    mutationFn: ({contract, operation}: ContractAction) =>
      operation === "delete"
        ? api<Contract | null>(`/companies/${company!.id}/contracts/${contract.id}`, {method: "DELETE"})
        : api<Contract | null>(`/companies/${company!.id}/contracts/${contract.id}/${operation}`, {method: "POST"}),
    onSuccess: () => {
  ...
  return <div className="flex flex-col gap-5">
        ...
        : rows.length === 0 ? <div className="flex flex-col items-center gap-2 py-20"><FileSignature size={28}/><b>Niciun contract</b><span className="text-sm text-[var(--text-muted)]">Definește termenii comerciali care vor alimenta facturile recurente.</span></div>
          : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="bg-[var(--bg-muted)] text-left text-xs uppercase text-[var(--text-muted)]"><tr><th className="p-3">Contract</th><th className="p-3">Client</th><th className="p-3">Valabilitate</th><th className="p-3">Model</th><th className="p-3">Stare</th><th className="p-3 text-right">Acțiuni</th></tr></thead><tbody>{rows.map((contract) => <tr key={contract.id} className="border-t border-[var(--border)]">
            ...
            <td className="p-3"><div className="flex justify-end gap-1"><Tooltip delay={300}><Button isIconOnly size="sm" variant="ghost" aria-label={`${contract.status === "draft" ? "Editează" : "Vezi"} contractul ${contract.number}`} onPress={() => setEditing(contract)}><Pencil size={15}/></Button><Tooltip.Content>{contract.status === "draft" ? "Editează contractul" : "Vezi contractul"}</Tooltip.Content></Tooltip>
              ...
              {contract.status === "draft" && can("contract.manage") ? <Tooltip delay={300}><Button isIconOnly size="sm" variant="ghost" aria-label={`Șterge contractul ${contract.number}`} onPress={() => setConfirmation({contract, operation: "delete"})}><Trash2 size={15}/></Button><Tooltip.Content>Șterge definitiv ciorna</Tooltip.Content></Tooltip> : null}
    ...
    <ConfirmDialog
      ...
      title={confirmation?.operation === "activate" ? `Activezi contractul ${confirmation.contract.number}?` : confirmation?.operation === "end" ? `Închei contractul ${confirmation.contract.number}?` : confirmation?.operation === "delete" ? `Ștergi definitiv ciorna ${confirmation.contract.number}?` : `Arhivezi contractul ${confirmation?.contract.number ?? ""}?`}
      description={confirmation?.operation === "activate" ? "Contractul va deveni activ, iar termenii versiunii curente nu vor mai putea fi editați direct. Modificările ulterioare se fac printr-o versiune nouă." : confirmation?.operation === "end" ? "Contractul va trece în starea Încheiat și nu va mai putea fi reactivat din aplicație." : confirmation?.operation === "delete" ? "Contractul, toate versiunile și toate liniile sale vor fi șterse definitiv. Acțiunea nu poate fi anulată." : "Contractul va fi arhivat și scos din fluxul curent. Istoricul contractual și facturile existente rămân păstrate, iar contractul va putea fi restaurat ulterior."}
      confirmLabel={confirmation?.operation === "activate" ? "Activează contractul" : confirmation?.operation === "end" ? "Încheie contractul" : confirmation?.operation === "delete" ? "Șterge definitiv" : "Arhivează contractul"}
      tone={confirmation?.operation === "archive" || confirmation?.operation === "delete" ? "danger" : "warning"}
...
function ContractVersionEditor({companyId, companyIsVatPayer, contract, version, onSaved}: {companyId: string; companyIsVatPayer: boolean; contract: Contract; version: ContractVersion; onSaved: () => void}) {
  ...
  const [form, setForm] = useState<ContractForm>(() => ({
    ...
    currency_id: version.currency_id, payment_terms_days: String(version.payment_terms_days), default_hours_per_day: version.default_hours_per_day ?? "8.00",
  ...
  return <><div className="grid gap-4 p-5 sm:grid-cols-2">
    ...
    <Field label="Monedă"><AppSelect name="currency_id" ariaLabel="Monedă" value={form.currency_id} onChange={(value) => set("currency_id", value)} options={activeCurrencies.map((currency) => ({id: currency.id, label: `${currency.code} — ${currency.name}`}))}/></Field>
...
function ContractModal({companyId, companyIsVatPayer, contract, onClose, onSaved}: {companyId: string; companyIsVatPayer: boolean; contract: Contract | null; onClose: () => void; onSaved: () => void}) {
  ...
  const [form, setForm] = useState<ContractForm>(() => contract && contract.current_version && line ? {
    ...
    currency_id: contract.current_version.currency_id, payment_terms_days: String(contract.current_version.payment_terms_days), default_hours_per_day: contract.current_version.default_hours_per_day ?? "8.00",
  ...
  const missingVatConfiguration = companyIsVatPayer && !vatProfiles.isLoading && activeVatProfiles.length === 0;
  useEffect(() => {
    if (contract || activeCurrencies.length === 0 || activeCurrencies.some((currency) => currency.id === form.currency_id)) return;
    const preferred = activeCurrencies.find((currency) => currency.is_local) ?? activeCurrencies[0];
    setForm((current) => ({...current, currency_id: preferred.id}));
  }, [activeCurrencies, contract, form.currency_id]);
  useEffect(() => {
    if (form.vat_profile_id || vatProfiles.isLoading) return;
    const profile = activeVatProfiles.find((item) => item.is_default) ?? activeVatProfiles[0];
    if (profile) setForm((current) => ({...current, vat_profile_id: profile.id, vat_rate: profile.rate, vat_category: profile.vat_category, vat_exemption_code: profile.vat_exemption_code, vat_exemption_reason: profile.vat_exemption_reason}));
  ...
  const payload = {customer_id: form.customer_id, number: form.number.trim(), name: form.name.trim(), signed_on: form.signed_on, starts_on: form.starts_on, ends_on: form.ends_on || null, version: {
    effective_from: contract?.current_version?.effective_from ?? form.starts_on, currency_id: form.currency_id, payment_terms_days: Number(form.payment_terms_days), locale: "ro", timezone: "Europe/Bucharest", working_weekdays: [1,2,3,4,5], holiday_calendar_code: "RO", default_hours_per_day: form.default_hours_per_day || null, notes: null,
  ...
  const contractRequiredReason = requiredFieldsReason([
    ...
    {label: "monedă", missing: !form.currency_id},
    {label: "denumire poziție", missing: !form.line_name.trim()},
    {label: "profil TVA", missing: !form.vat_profile_id},
  ]);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/45 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-3xl rounded-2xl bg-[var(--surface)] shadow-[var(--shadow-lg)]"><header className="flex items-center justify-between border-b border-[var(--border)] p-5"><div><h2 className="font-semibold">{contract ? `${contract.number} · ${contract.name}` : "Contract nou"}</h2><p className="text-xs text-[var(--text-muted)]">Termenii activi sunt versionați; recurența păstrează versiunea exactă.</p></div><Button isIconOnly variant="ghost" onPress={onClose}>×</Button></header>
    <div className="grid gap-4 p-5 sm:grid-cols-2">
      ...
      <Field label="Monedă"><AppSelect name="currency_id" ariaLabel="Monedă" value={form.currency_id} isDisabled={!editable} onChange={(value) => set("currency_id", value)} options={activeCurrencies.map((currency) => ({id:currency.id,label:`${currency.code} — ${currency.name}`}))}/></Field>
      ...
      {missingVatConfiguration ? <div className="rounded-xl bg-[var(--warning-soft)] p-3 text-sm text-[var(--warning)] sm:col-span-2">Firma este plătitoare de TVA, dar nu are niciun profil TVA activ. Configurează profilurile în Setări înainte de salvare.</div> : null}
      {preview.data ? <div className="rounded-xl bg-[var(--bg-muted)] p-4 text-sm sm:col-span-2"><b>Calcul curent</b><div>{preview.data.data.working_days.working_days} zile lucrătoare · {preview.data.data.lines[0]?.quantity} ore/unități · {money(preview.data.data.totals.total_cents, activeCurrencies.find((currency) => currency.id === form.currency_id)?.code ?? contract?.current_version?.currency ?? "RON")}</div><p className="mt-1 text-xs">{preview.data.data.lines[0]?.description}</p></div> : null}
