...
export function FiscalVaultPage() {
  const {company} = useCompany(); const grid = useServerDataGridState({defaultSort: DEFAULT_SORT, sortColumns: ["archived_at", "issue_date", "supplier_name", "document_number"]});
  const items = useQuery({queryKey: ["fiscal-vault", company?.id, grid.page, grid.debouncedSearch, grid.apiSort], enabled: Boolean(company?.id), placeholderData: (previous) => previous, queryFn: () => api<FiscalVaultItem[]>(`/companies/${company!.id}/vault${listQuery({page: grid.page, perPage: 20, sort: grid.apiSort, filter: grid.debouncedSearch ? {document_number: {contains: grid.debouncedSearch}} : {}})}`)});
