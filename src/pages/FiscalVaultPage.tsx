...
export function FiscalVaultPage() {
  ...
  const columns = useMemo<DataGridColumn<FiscalVaultItem>[]>(() => [
    ...
    {id: "actions", header: "", align: "end", minWidth: 70, cell: (item) => can("fiscal_vault.download") ? <Button isIconOnly size="sm" variant="ghost" aria-label={`Descarcă ${item.document_number ?? "documentul"}`} isDisabled={!item.original || download.isPending} onPress={() => download.mutate(item)}><Download size={16}/></Button> : null},
  ], [can, download]);
